...
function(input, output) {
  ...
  output$phonePlot <- renderPlot({
    ...
    pon = paste(
      "SELECT Round(bytes/(1024*1024*1024),3) as TB,filename,date from serverdata where filename like '",
      input$region,
      "'",
      sep = ""
    )
    ...
    plot(
      ...
      col = "red",
      xlab = "Month",
      ylab = "Space Usage in TB",
      main = input$region,
      axes = TRUE,
      frame = TRUE,
      xaxt = "n",
      yaxs = "i",
      ylim = c(0, max(t[, "TB"]) * 1.10)
    )
    label_x <- t[, "date"]
  ...
  output$line <- renderPlot({
    plot_data = paste(
      "SELECT ROUND(bytes/(1024*1024*1024),3) as TB,filename,date from gencore where filename like '",
      input$region2,
      "'",
      sep = ""
    )
  ...
  output$view <- renderTable({
    po = paste(
      "SELECT bytes/(1024*1024*1024) as TB,filename,date from serverdata where filename like '",
      input$region,
      "' order by date desc",
      sep = ""
    )
